...
@Autonomous(name = "auto drop", group = "Wallace")
@Disabled
public class autodrop extends LinearOpMode {
	...
	grabber.grab();
	arm.move(0.4);
	while(arm.getCurrentPosition() < 1760)
	    {
		sleep(1);
	    }
	arm.move(0.05);
            ...
            while (distance_back.getDistanceMM() > 500) {
                sleep(1);
            }
	    ...
	    left_turn_counter(1350);
	slide.move(0.4);
	while(slide.getCurrentPosition() < 2980)
	    {
		sleep(1);
	    }
	slide.move(0.05);
	moveRobot_forward(DRIVE_SPEED,0,12);
	grabber.release();
	    while(!gamepad1.a)
		{
		    sleep(1);
...
//            orientation = imu.getRobotYawPitchRollAngles();
    ...
    void left_turn_counter(long ticks) {
	telemetry.addData("oldpos: ", " %10d",leftFrontDrive.getCurrentPosition() );
	...
	while(	leftFrontDrive.getCurrentPosition() > toposition)
        ...
        leftBackDrive.setPower(0);
        rightBackDrive.setPower(0);
	telemetry.addData("ticks: ", " %10d", ticks);
	telemetry.addData("currpos: ", " %10d",leftFrontDrive.getCurrentPosition() );
	telemetry.addData("topos: ", " %10d",toposition );
            telemetry.update();
	    // while(!gamepad1.a)
	    // 	{
	    // 	    sleep(1);
